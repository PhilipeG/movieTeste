...
// Um elemento position:fixed é posicionado em relação ao viewport, A MENOS que um
// ancestral estabeleça um containing block (transform, perspective, filter,
// will-change desses, ou contain). Nesse caso o translate do cursor deixa de
// mapear para coordenadas do viewport, então medimos e compensamos o offset.
...
const TargetCursor = ({
  ...
  useEffect(() => {
    if (isMobile || !mounted || !cursorRef.current) return
    ...
    // Cores interpoladas via CSS transition (suporta var(--primary) do tema)
    const setCursorColors = (color: string) => {
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      activeTarget = target
      if (showOnTargetOnly) {
        gsap.to(cursorRef.current, { autoAlpha: 1, duration: 0.2, ease: "power2.out", overwrite: "auto" })
      }
      const corners = Array.from(cornersRef.current)
      ...
      if (cursorColorOnTarget) {
        setCursorColors(cursorColorOnTarget)
      }
      ...
      const leaveHandler = () => {
        ...
        if (showOnTargetOnly && cursorRef.current) {
          gsap.to(cursorRef.current, { autoAlpha: 0, duration: 0.25, ease: "power2.out", overwrite: "auto" })
        }

        if (cursorColorOnTarget) {
          setCursorColors(cursorColor)
        }
