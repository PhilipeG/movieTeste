...
import { Heart, Check, X, Trash2, Star, Plus, CalendarDays, Clock, Users, Clapperboard } from "lucide-react"
...
export default function MovieCard({
  ...
  const [details, setDetails] = useState<MovieDetails | null>(() => detailsCache.get(movie.id) ?? null)
          ...
          <div className="relative z-10 flex flex-col h-full p-4">
            ...
            <div className="flex-1 min-h-0 flex flex-col justify-evenly gap-2 text-[13px] text-muted-foreground">
              ...
              <div className="flex items-start gap-1.5">
                ...
                <span className="line-clamp-2 leading-snug">{genres ? genres.join(" • ") : "…"}</span>
              </div>

              <div className="flex items-start gap-1.5">
                <Users className="w-4 h-4 text-primary shrink-0 mt-0.5" />
                <span className="line-clamp-3 leading-snug">{cast ? cast.join(", ") : "…"}</span>
              </div>
                ...
                <div className="flex items-center gap-5">
                  {ratings.anak !== undefined && (
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-[11px] font-bold tracking-wider uppercase text-red-500">Anak</span>
                      <span className={`text-xl font-bold leading-none ${getRatingColor(ratings.anak)}`}>
                        {ratings.anak}
                      </span>
                    </div>
                  )}
                  {ratings.silvio !== undefined && (
                    <div className="flex items-baseline gap-1.5">
                      <span className="text-[11px] font-bold tracking-wider uppercase text-blue-500">Silvio</span>
                      <span className={`text-xl font-bold leading-none ${getRatingColor(ratings.silvio)}`}>
                        {ratings.silvio}
                      </span>
                    </div>
                  )}
