...
function ScoreRating({ value, onChange, label }: any) {
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    let newValue = Math.ceil(percent * 20) / 2
    if (newValue > 10) newValue = 10
    if (newValue === 0 && x > 0) newValue = 0.5
  ...
  return (
    <div className="flex flex-col gap-2 bg-secondary/30 p-3 rounded-xl border border-white/5 transition-colors">
      <div className="flex justify-between items-center">
        ...
        <span className={`text-lg font-bold transition-colors ${displayValue > 0 ? "text-white" : "text-muted-foreground"}`}>
      ...
      <div 
        ref={containerRef}
        className="flex gap-1 h-4 w-full cursor-pointer py-1"
      ...
      >
        {Array.from({ length: 10 }).map((_, i) => {
          ...
          let backgroundStyle = {}
          let borderClass = "bg-white/10"

          if (displayValue >= blockIndex) {
            borderClass = currentColorClass
          } else if (displayValue === blockIndex - 0.5) {
            backgroundStyle = {
              background: `linear-gradient(90deg, ${currentHex} 50%, rgba(255,255,255,0.1) 50%)`
            }
            borderClass = ""
          }
          ...
          return (
            <div
              key={blockIndex}
              className={`flex-1 rounded-sm h-full ${borderClass} transition-none`}
          ...
          <div className="flex gap-2 p-1 mb-4 overflow-x-auto">
            {(["sinopse", "info", "galeria"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`cursor-pointer flex-1 px-4 py-2 text-sm font-medium rounded-xl transition-all duration-200 active:scale-95 ${
                  tab === t
                ...
                {loadingDetails ? (
                  ...
                  <>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm text-muted-foreground mb-1">Gêneros</p>
                        <div className="flex flex-wrap gap-2">
                          {details?.genres?.map((g: Genre) => (
                            <span 
                              key={g.id} 
                              className="text-xs font-medium px-3 py-1 rounded-lg bg-primary/10 text-primary border border-primary/20"
                            >
                              {g.name}
                            </span>
                          )) || "N/A"}
