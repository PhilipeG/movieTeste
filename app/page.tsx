...
export default function Home() {
  ...
  const [activeFilters, setActiveFilters] = useState<{ genreId: string | null; year: string | null; minRating: number | null; }>({ genreId: null, year: null, minRating: null })
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (showFilters && filterRef.current && !filterRef.current.contains(event.target as Node) && filterBtnRef.current && !filterBtnRef.current.contains(event.target as Node)) {
  ...
  const fetchMovies = async (resetPage = true) => {
    if (resetPage) { setLoading(true); setCurrentPage(1) } else { setLoadingMore(true) }

    try {
      const data = await getPopularMovies(resetPage ? 1 : currentPage + 1, activeFilters)
      if (resetPage) { setMovies(data) } else {
        const uniqueMovies = data.filter((newMovie) => !movies.some((existing) => existing.id === newMovie.id))
        setMovies((prev) => [...prev, ...uniqueMovies])
        setCurrentPage((prev) => prev + 1)
      }
    } catch (error) { toast.error("Erro ao buscar filmes") } finally { setLoading(false); setLoadingMore(false) }
  }

  const applyFilters = () => { setCurrentView("popular"); fetchMovies(true); setShowFilters(false) }

  const clearFilters = () => {
    setActiveFilters({ genreId: null, year: null, minRating: null })
    setLoading(true)
    getPopularMovies(1, { genreId: null, year: null, minRating: null }).then(data => { setMovies(data); setLoading(false) })
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <div className="container mx-auto px-4 py-8 max-w-7xl">
              ...
              <Menu as="div" className="relative">
                ...
                <Transition as={Fragment} enter="transition ease-out duration-200" enterFrom="transform opacity-0 scale-95" enterTo="transform opacity-100 scale-100" leave="transition ease-in duration-150" leaveFrom="transform opacity-100 scale-100" leaveTo="transform opacity-0 scale-95">
                  <MenuItems className="absolute right-0 mt-2 w-56 glass rounded-xl shadow-xl z-20 focus:outline-none overflow-hidden">
                    <div className="p-2">
                      <MenuItem>{({ active }) => (<button onClick={displayFavoriteMovies} className={`${active ? "bg-secondary/50" : ""} cursor-pointer flex items-center gap-3 w-full px-4 py-3 text-sm text-foreground rounded-lg transition-colors`}><Heart className="w-4 h-4 text-primary" />Favoritos{favorites.length > 0 && (<span className="ml-auto text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">{favorites.length}</span>)}</button>)}</MenuItem>
                      <MenuItem>{({ active }) => (<button onClick={displaySeenMovies} className={`${active ? "bg-secondary/50" : ""} cursor-pointer flex items-center gap-3 w-full px-4 py-3 text-sm text-foreground rounded-lg transition-colors`}><Eye className="w-4 h-4 text-primary" />Já Assistidos{seenMovies.length > 0 && (<span className="ml-auto text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">{seenMovies.length}</span>)}</button>)}</MenuItem>
                      <MenuItem>{({ active }) => (<button onClick={displayRoulette} className={`${active ? "bg-secondary/50" : ""} cursor-pointer flex items-center gap-3 w-full px-4 py-3 text-sm text-foreground rounded-lg transition-colors`}><Trophy className="w-4 h-4 text-primary" />Roleta{rouletteMovies.length > 0 && (<span className="ml-auto text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">{rouletteMovies.length}</span>)}</button>)}</MenuItem>
                    </div>
                  </MenuItems>
                </Transition>
              </Menu>
        ...
        {bannerMovies.length > 0 && currentView === "popular" && (<HeroCarousel movies={bannerMovies} onSelect={setSelectedMovie} />)}
        ...
        <div className="relative flex items-center gap-3 mb-6 z-10">
          ...
          {currentView === "seen" && (<button onClick={handleOpenStats} className="ml-2 p-2 rounded-lg bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground transition-all cursor-pointer group" title="Estatísticas"><BarChart2 className="w-4 h-4 group-hover:scale-110 transition-transform" /></button>)}
                ...
                {showFilters && (
                    <div ref={filterRef} className="absolute top-full left-0 mt-3 w-72 md:w-96 glass p-5 rounded-2xl shadow-2xl border border-white/10 animate-in slide-in-from-top-2 fade-in duration-200 z-50">
                        ...
                        <div className="space-y-5">
                            <div><label className="text-xs font-medium text-muted-foreground mb-2 block flex items-center gap-1"><Film className="w-3 h-3" /> Gênero</label><FilterSelect value={activeFilters.genreId} onChange={(val: any) => setActiveFilters(prev => ({ ...prev, genreId: val }))} options={genreOptions} icon={Film} placeholder="Todos os gêneros" /></div>
                            <div><label className="text-xs font-medium text-muted-foreground mb-2 block flex items-center gap-1"><Calendar className="w-3 h-3" /> Ano de Lançamento</label><FilterSelect value={activeFilters.year} onChange={(val: any) => setActiveFilters(prev => ({ ...prev, year: val }))} options={yearOptions} icon={Calendar} placeholder="Todos os anos" /></div>
                            <div><label className="text-xs font-medium text-muted-foreground mb-2 block flex justify-between items-center"><span className="flex items-center gap-1"><Star className="w-3 h-3" /> Nota Mínima</span><span className="text-primary font-bold bg-primary/10 px-2 py-0.5 rounded-md">{activeFilters.minRating || 0}+</span></label><input type="range" min="0" max="9" step="1" value={activeFilters.minRating || 0} onChange={(e) => setActiveFilters(prev => ({ ...prev, minRating: Number(e.target.value) || null }))} className="w-full accent-primary h-2 bg-secondary/50 rounded-lg appearance-none cursor-pointer" /><div className="flex justify-between text-[10px] text-muted-foreground mt-1 px-1"><span>0</span><span>5</span><span>9</span></div></div>
                            <button onClick={applyFilters} className="cursor-pointer w-full bg-primary/10 text-primary border border-primary/20 hover:bg-primary hover:text-primary-foreground py-2.5 rounded-xl font-medium shadow-sm transition-all active:scale-95 mt-2">Aplicar Filtros</button>
                        </div>
                    </div>
                )}
            </div>
        ...
        {currentView === "roulette" ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">{Array.from({ length: 18 }).map((_, i) => (<div key={i} className="aspect-[2/3] rounded-xl bg-card animate-pulse" />))}</div>
        ) : movies.length > 0 ? (
          <>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            {(activeFilters.genreId || activeFilters.year || activeFilters.minRating) && (<button onClick={clearFilters} className="mt-4 text-primary hover:underline cursor-pointer">Limpar Filtros</button>)}
          </div>
        )}
